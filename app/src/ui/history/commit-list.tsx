...
import { DragAndDropIntroType } from './drag-and-drop-intro-type'
...
> {
  ...
  public constructor(props: ICommitListProps) {
    ...
    const allDragAndDropIntros = [
      DragAndDropIntroType.CherryPick,
      DragAndDropIntroType.Squash,
      DragAndDropIntroType.Reorder,
    ]

    const remainingDragAndDropIntros = allDragAndDropIntros.filter(
  ...
  private renderCherryPickIntroPopover() {
    ...
    return (
      <Popover caretPosition={PopoverCaretPosition.LeftTop}>
        ...
        <h3>
          Drag and drop your commits!
          <span className="call-to-action-bubble">New</span>
        </h3>
        <p>{this.getDragAndDropIntroText()}</p>
        <div>
  ...
  private getDragAndDropIntroText() {
    if (this.state.remainingDragAndDropIntros.length === 0) {
      return null
    }
    ...
    switch (intro) {
      case DragAndDropIntroType.CherryPick:
        return `Copy commits to another branch by dragging and dropping them
        onto a branch in the branch menu, or by right clicking on a commit.`
      case DragAndDropIntroType.Squash:
        return `Squash commits by dragging and dropping them onto another commit,
        or by right clicking on multiple commits.`
      case DragAndDropIntroType.Reorder:
        return `Reorder commits to tidy up your history by dragging and dropping
        them in a different position.`
      default:
        assertNever(intro, `Invalud drag & drop intro type ${intro}`)
    }
  }
